...
function getData() {
	...
	$.get("/truck/get?tbStartIndex=" + tbStartIndex + "&tbPageSize=10", null, receiveData);
}
...
$( document ).on( "pageinit", "#myPage", function() {    
    ...
    $( "#btn-create-element" ).on( "click", function( event ) {
    	$.post("/truck/add?" + $("#form-create-element").serialize(), null, function() {getData();});
    	$("#form-create-element")[0].reset();
		$(this).parent().parent().parent().parent().popup("close");
	});
    
    $( "#btn-modify-element" ).on( "click", function( event ) {
    	$.post("/truck/update?" + $('#form-modify-element').serialize(), null, function() {getData();});
    	$(this).parent().parent().parent().parent().popup("close");
	});
    
    $( "#btn-delete-element" ).on( "click", function( event ) {
		$.post("/truck/remove?" + $('#form-delete-element').serialize(), null, function() {getData();});
		$(this).parent().parent().parent().parent().popup("close");
	});
