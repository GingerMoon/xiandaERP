function receiveData(data) {
	...
	$(".btn-edit").on("click", function(event) {
		...
		$("#modify-element-date")[0].value = $(this).parent().parent().children()[2].textContent;
    ...
    $(".btn-delete").on("click", function(event) {
    	...
    	$("#delete-element-date")[0].value = $(this).parent().parent().children()[2].textContent;
...
function searchByName( e, data ) {
    ...
    $( document).on( "click", "#autocomplete_searchByName li", function() {
		...
		$.get("/truck/getById?id="+id, null, receiveData);
...
$.get("/truck/get?tbStartIndex=0&tbPageSize=10", null, receiveData);
    ...
    $( "#element-btn-display-page" ).on( "click", function( event ) {
		...
		StartIndex = page;
		$.get("/truck/get?tbStartIndex=" + (page-1) + "&tbPageSize=10", null, receiveData);
		$(this).parent().popup("close");
	});
    
    $( "#btn-create-element" ).on( "click", function( event ) {
    	$.post("/truck/add?" + $("#form-create-element").serialize(), null, receiveData);
    	$("#form-create-element")[0].reset();
		$(this).parent().parent().parent().parent().popup("close");
	});
    
    $( "#btn-modify-element" ).on( "click", function( event ) {
    	$.post("/truck/update?" + $('#form-modify-element').serialize(), null, receiveData);
    	$(this).parent().parent().parent().parent().popup("close");
	});
    
    $( "#btn-delete-element" ).on( "click", function( event ) {
		$.post("/truck/remove?" + $('#form-delete-element').serialize(), null, receiveData);
		$(this).parent().parent().parent().parent().popup("close");
	});
    
    // export to a excel file
    $( "#pop-up-btn-export-excel" ).on( "click", function( event ) {
    	document.location.href='./truck/exportExcel';	
	});
