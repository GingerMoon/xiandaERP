...
function searchByName( e, data ) {
    ...
    $( document).on( "click", "#autocomplete_searchByName li", function() {
		...
		$.get("/customer/getById?id="+id, null, receiveData);
...
$.get("/customer/get?tbStartIndex=0&tbPageSize=10", null, receiveData);
    ...
    $( "#element-btn-display-page" ).on( "click", function( event ) {
		...
		StartIndex = page;
		$.get("/customer/get?tbStartIndex=" + (page-1) + "&tbPageSize=10", null, receiveData);
		$(this).parent().popup("close");
	});
    
    $( "#btn-create-element" ).on( "click", function( event ) {
    	var name = $("#create-element-name").val();
    	var description = $("#create-element-description").val();
    	var state = $("#create-element-state").val();
    	$.post("/customer/add?" + $("#form-create-element").serialize(), null, receiveData);
    	$("#form-create-element")[0].reset();
		$(this).parent().parent().parent().parent().popup("close");
	});
    
    $( "#btn-modify-element" ).on( "click", function( event ) {
    	var id = $("#modify-element-id").val();
    	var name = $("#modify-element-name").val();
    	var description = $("#modify-element-description").val();
    	var state = $("#modify-element-state").val();
    	$.post("/customer/update?" + $('#form-modify-element').serialize(), null, receiveData);
    	$(this).parent().parent().parent().parent().popup("close");
	});
    
    $( "#btn-delete-element" ).on( "click", function( event ) {
    	var id = $("#delete-element-id").val();
		$.post("/customer/remove?" + $('#form-modify-element').serialize(), null, receiveData);
		$(this).parent().parent().parent().parent().popup("close");
	});
    
    // export to a excel file
    $( "#pop-up-btn-export-excel" ).on( "click", function( event ) {
    	document.location.href='./customer/exportExcel';	
	});
